...
- 产品内容不随技能附带固定产品库：运行期由 `scripts/fetch_erp_catalog.py` 从 ERP §6 按需切片拉取（阶段 E 按已确认业务线、目标空间或需求关键词触发，可经 `--keywords` 客户端收窄），生成 `assets/erp-runs/<run_id>/snapshot.json` 并 SHA-256 指纹化；该快照即本次运行的"标准产品库"等价物，记入 `source_manifest.content_fingerprint_refs`。图片为 ERP `image_url` 远程引用（不下载、不本地核验）。阶段 A 在 `template_profile` 中记录 `产品图片` 列、行高、打印区域和样式策略；不在此阶段将图片映射到项目行，也不得把图片当作产品身份或需求证据。
...
- 业务线确认：从需求证据推导项目涉及的业务线集合 `business_lines`（取值为映射层 `query_routing.business_lines` 的键，如 `生涯`/`科创`），并为产品类需求项标注 `business_line` 归属；`business_lines` 是阶段 E 拉数切片的入口依据。`business_lines` 非空且经需求基线闸门显式确认后，基线方可 `approved`；取值不在路由表内时记 warning 并在闸门澄清。
...
- 功能画像的 `core_functions`/`secondary_functions` 是阶段 E 语义匹配的需求侧输入之一；`capacity`/`area`/安装条件承担数量推导与物理适配依据。功能画像必须在候选匹配之前完成。
- 空间规划建议必须同时说明功能、总面积、人数和项目重点四方面的依据；缺失项应进入 `unresolved_items`，不得以常识补齐。
- 确认面积、人数、活动强度、班级数、课时数、参赛队数、安装条件、共享关系和工程量依据；未知值保持为空并进入澄清，不得猜测。
...
- 候选匹配采用多信号分层（universal-rules §4）：按已确认业务线经 `query_routing.business_lines` 切片拉数，以需求功能词（经 `keyword_aliases` 扩展）对照候选的小类名/分组名、`function_tags`、产品名与 `product_intro` 进行语义匹配，记录 `semantic_match_reason` 与 `confidence`；`source_spaces` 仅作参考证据。再结合目标空间的功能、对象、活动、容量和条件判断空间适配与数量。小类名/分组名、`function_tags`、产品名、`product_intro` 四者皆无有效语义时，必须生成数据质量问题并阻断受影响产品进入正式候选或正式清单；只有主锚点信号齐全但语义相关性不足时才可标记为弱匹配。
- 产品行必须来自运行期 ERP 快照（`assets/erp-runs/<run_id>/snapshot.json`）；非产品行必须有规则或批准依据。
...
- 仅在产品方案闸门通过后，才可按 `internal_product_mapping.catalog_product_key` 为已选行插入图片。图片是 ERP `image_url` **远程引用**（`role=primary`、`confidence=remote`、`match_rule=erp-image-url-v1`、`locally_verified=false`），不下载、不本地 SHA-256 核验、不嵌入本地文件；在映射和追溯审计中记录 `image_ref.url`、`catalog_product_key` 与输出单元格。禁止按名称重新匹配、使用外部链接、网络搜索或生成图。没有可用 ERP 图片时留空并披露 warning；除非已批准输出要求把图片设为必填字段，否则不阻断。
...
- 运行期 ERP 快照缺失、不可读取、SHA-256 指纹无法生成或与发布绑定指纹不一致时，立即停止选品；只输出问题说明和所需资料，不得用历史记忆、网络信息或假设替代。薄映射层 `product-mapping.json` 同样遵循此规则。仅缺少 `business_version`/`effective_date` 时记录 warning，不停止选品。
...
- 运行期 ERP 快照或薄映射层 `product-mapping.json` 的 SHA-256 `content_fingerprint` 发生变化时，保留旧版 `candidate_plan`、`internal_product_mapping` 和 `traceability_audit`，基于新指纹重新执行候选配置、产品确认与追溯审计，并标记指纹差异；不得覆盖旧版。
