...
| `business_lines` | 字符串数组 | 是 | 项目涉及的业务线集合；取值须为映射层 `query_routing.business_lines` 的键（如 `生涯`/`科创`）；`status=approved` 前必须非空且经需求基线闸门显式确认 |
...
| `business_line` | 字符串或空值 | 是 | 产品类需求归属的业务线；取值须为映射层 `query_routing.business_lines` 的键，用于阶段 E 切片拉数；`identity/budget/pricing/output` 等非产品类需求为 `null` |
...
| `business_line` | 字符串或空值 | 从来源需求继承的业务线归属；非产品行（engineering/service/material/allowance）可为空 |
...
| `source_refs` | 来源引用数组 | 产品行指向本次运行的 ERP 快照（固定资源）；非产品行指向规则或批准依据 |
| `catalog_product_key` | 字符串或空值 | 产品行必须为 ERP 快照的稳定 `product_key`（= ERP `product_no`）；非产品行为空。它用于唯一定位来源快照记录与图片引用，不能以 `product_identity_key` 或产品名称替代 |
| `name` | 字符串 | 产品行必须精确等于 ERP 快照标准 `product_name`；非产品行使用规则或批准依据中的标准品目名 |
...
| `input_versions` | 对象 | 是 | 必含 `requirement_baseline_version_ref`、`source_manifest_version_ref`、`template_profile_version_ref`、`space_plan_version_ref`、`candidate_plan_version_ref`，并绑定标准产品库、四份 references、`assets/project-intake-template.yaml`、默认/用户模板；固定资源和模板均记录路径与 SHA-256 `content_fingerprint`，intake asset 另含 `schema_version=1.1`。若输出使用产品图，额外记录每张图的 `catalog_product_key`（= product_no）、`image_ref.url`、`image_ref.locally_verified=false`、`image_ref.match_rule=erp-image-url-v1` 与输出单元格；不记录本地路径或本地 SHA-256 |
...
`template_profile` 必须额外记录 `产品图片` 列的存在、位置、行高策略和打印区域影响。`internal_product_mapping` 的产品行必须额外记录稳定 `catalog_product_key`（= ERP `product_no`）；若插入图片引用，还必须记录 `image_ref.url`、`image_ref.locally_verified=false`、`image_ref.match_rule=erp-image-url-v1`、源表图片单元格和输出图片单元格。`product_identity_key` 或产品名称均不能替代 `catalog_product_key` 来绑定图片；图片为 ERP 远程链接，不下载、不本地 SHA-256 核验。
