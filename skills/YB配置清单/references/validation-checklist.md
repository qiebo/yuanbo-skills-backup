...
- [ ] `requirement_baseline.status=approved`，但 `business_lines` 为空、含未在映射层 `query_routing.business_lines` 中定义的取值，或未经需求基线闸门显式确认。
...
- [ ] `selected_for_output=true` 的产品行缺少 ERP 快照来源、快照 SHA-256 指纹或稳定记录定位（`product_key`=product_no / `erp_id`）。
- [ ] 已插入的产品图片引用未通过稳定 `catalog_product_key`（=product_no）绑定，缺少 `role=primary`、`confidence=remote`、`match_rule=erp-image-url-v1` 或 `locally_verified=false`，使用了非 ERP `image_url` 来源（外部链接/网络检索/生成图），或图片 URL/来源快照未写入 `internal_product_mapping` 与追溯审计。
- [ ] 正式输出涉及的产品，其小类名/分组名（`category`/`product_group`）、`function_tags`、产品名、`product_intro` 四者皆无有效语义，或未生成对应质量问题而进入正式清单（阻断）。`source_spaces` 缺失不属于语义缺失，不生成质量问题、不阻断。`product_intro` 仅在 ERP 的 `specifications`/`remark`/`product_advantages` 全部为空时才生成质量问题（warning 级，披露但不阻断发布）。
...
- [ ] 固定资源（薄映射层 `product-mapping.json` 或运行期 ERP 快照 `snapshot.json`）内容不可读、SHA-256 指纹无法生成，或实际指纹与发布绑定指纹不一致。
...
- [ ] 使用暂估面积或暂估工程量。
- [ ] 关键语义信号齐全但 `confidence=low` 的弱语义匹配；小类名/分组名、`function_tags`、产品名、`product_intro` 四者皆缺不是警告，必须按硬错误阻断受影响产品。
- [ ] 候选行 `business_line` 取值不在映射层 `query_routing.business_lines` 中（新业务线扩展前的提示）；或通用物资（如桌椅/触控屏）经关键词全库搜索从其他业务线大类命中——跨线命中须在交付说明披露来源类目。
- [ ] 缺少采购历史或供应历史，无法进行历史可得性佐证。
- [ ] 价格来源日期过旧，超出项目定义的时效阈值。
- [ ] 可选功能未覆盖。
- [ ] 固定资源未提供 `business_version` 或 `effective_date`；不得编造，且缺失本身不阻断。
- [ ] 已选产品没有可用的 ERP `image_url` 远程引用；留空图片单元格，并披露原因（图片为远程引用、非本地核验）。除非已批准输出要求把图片设为必填字段，否则不阻断发布。
