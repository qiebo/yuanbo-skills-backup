...
## Production Quality Rules From Editable PPT Reviews

These rules are mandatory when generating or optimizing business presentation decks that will be handed to a user as editable `.pptx` files.

### Design Diversity & Richness Guidelines (MANDATORY)

When generating SVG pages, the Executor MUST follow these design principles to avoid monotonous, sparse, or visually bland slides:

**1. Layout Diversity — Avoid Page Homogeneity**
- **NEVER** use the same layout template for more than 2 consecutive pages.
- Alternate between at least 3 distinct layout patterns across the deck: e.g., left-image-right-text, top-title-bottom-cards, full-bleed background with overlay text, split-screen comparison, centered focal point with radial cards, timeline horizontal flow, etc.
- For content pages describing multiple zones/areas/modules, vary the card arrangement: single row → staggered grid → 2+1 asymmetric → circular cluster → sidebar + main content.
- If the deck has 8+ content pages, no fewer than 4 distinct layout families should appear.

**2. Content Density — Avoid Visual Emptiness**
- Every page must carry meaningful visual weight. If a page feels "sparse" (large blank areas, only a title + 2 bullets), add supporting elements: icons, data callouts, small illustrations, quote blocks, process steps, or related image thumbnails.
- Use the full canvas. For 16:9 decks, aim for 70–85% content coverage; avoid slides with <50% content area utilization.
- When describing a physical space or installation, include: photos/renders of the space, equipment highlights, usage scenarios, and outcome/benefit callouts — never just a title and a short paragraph.

**3. Icon & SVG Graphic Usage — Enhance Visual Communication**
- **Actively use icons** from the built-in icon library (`templates/icons/`) to annotate headings, bullet points, card headers, and process steps.
- For every major concept or category, pair it with a distinct icon — do not leave text-only blocks when an icon can add immediate visual recognition.
- Use small SVG graphics (shapes, connectors, decorative lines, badges) to break up text-heavy areas and create visual hierarchy.
- In card grids, each card should have an icon or a small illustration at the top — never bare text cards.

**4. Background Images for Space/Installation Pages — Elevate Aesthetic Quality**
- For pages describing physical spaces, laboratories, interactive zones, or architectural projects, **MUST** use a relevant background image (photograph, render, or atmospheric illustration) covering 30–100% of the slide area.
- Apply appropriate overlays (gradient masks, semi-transparent color blocks, or frosted-glass panels) to ensure text readability over the background image.
- If no exact photo is available, use a thematic stock image or an AI-generated scene that evokes the space's atmosphere — never leave these pages with a flat solid-color background.
- Background images should be high-resolution (recommend ≥1920×1080 source), properly cropped, and aligned with the page's focal point.

### Editable Text Structure

- Build text as paragraph-level or logical-block text boxes, not as one text element per visual line. A sentence, bullet group, card body, or paragraph that the user may later edit should usually be one text container with line breaks.
- Use separate text elements only for genuinely separate objects: slide titles, section labels, chips, KPI numbers, short badges, chart labels, and independent card headings.
- Avoid artificial line breaks that leave one Chinese character, one English word fragment, or one number alone on a line. If a line breaks badly, widen the box, reduce the wording, or lower font size slightly before splitting the text into more boxes.

### Module Alignment And Grouping

- Every repeated module/card must have a clear internal grid: title, icon, body, value/footnote, and any number or percentage must align to the same anchors across sibling cards.
- Text must be visually attached to the module it describes. Do not let labels drift toward neighboring modules, especially in two-column layouts, concept diagrams, and budget cards.
- For dense pages, treat each semantic group as a bounded block and verify the block as a whole: background shape, icon, heading, body, and note should move together and share consistent padding.

### Semantic Role Separation

- Do not repeat the same content under different conceptual labels. In framework, process, layered-logic, role-definition, capability-map, and budget-allocation pages, define each label's semantic role before drawing.
- Each major label should answer a different question, such as `why`, `what`, `how`, `where`, `who`, `sequence`, `level`, `function`, `outcome`, or `allocation`.
- If two labels contain the same terms or describe the same objects, rewrite one of them so the slide explains a hierarchy, relationship, or progression rather than duplicating a list.

### Budget Precision

- Before exposing exact amounts, infer the reporting level from the deck context or ask when uncertain.
- For executive or client-facing proposal slides, prefer rounded totals and percentages, such as `about 650k` and module share, unless the user explicitly requests itemized prices.
- Keep exact line-item amounts in internal calculations or appendices. In the main narrative, show budget scale, relative allocation, and delivery boundaries.
- Always distinguish quoted items, self-procured items, optional future modules, and items not included in the current quote.

### PowerPoint Render Review Gate

After export, do not rely only on SVG validation. Open or render the actual `.pptx` in PowerPoint and inspect a contact sheet plus the key slides at full size.

The final pass must check:

- no text overflow beyond cards, slide edges, or image overlays
- no clipped cards, cropped percentages, or off-slide labels
- no broken English words, awkward Chinese single-character lines, or one-word orphan lines
- no unwanted exact prices, dates, company labels, or footer text when the user asked to remove them
- visual rhythm matches the reference/template slides, especially section pages, directory pages, and case-study pages
- text objects remain practical to edit: paragraphs are not fragmented into many per-line text boxes
