...
| `catalog_product_key` | 字符串或空值 | 产品行必须为固定产品库的稳定 `product_key`；非产品行为空。它用于唯一定位图片，不能以 `product_identity_key` 或产品名称替代 |
...
`template_profile` 必须额外记录 `产品图片` 列的存在、位置、行高策略和打印区域影响。`internal_product_mapping` 的产品行必须额外记录稳定 `catalog_product_key`；若插入图片，还必须记录 `image_ref.path`、`image_ref.sha256`、源表图片单元格和输出图片单元格。`product_identity_key` 或产品名称均不能替代 `catalog_product_key` 来绑定图片。
