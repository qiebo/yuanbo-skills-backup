...
try:
    from validate_website_data import validate as validate_website_payload
except ImportError:
    validate_website_payload = None
...
def build_registry(excel, website, template):
    ...
    served_students = metrics["累计服务学生"]["value"]
    service_coverage_rate = served_students / student_accounts if student_accounts else None
    login_intensity_rate = total_logins / student_accounts if student_accounts else None
    ...
    append_row(
        ...
        "CORE-012",
        "登录人次覆盖强度",
        login_intensity_rate,
        "%",
        "公式生成",
        "Excel + 网站后台",
        "总登录人次 / 学生账号总数",
        "该指标反映登录人次强度，不能等同于去重学生覆盖率。",
        "CORE-004 / CORE-003",
    )
    append_row(
        core_rows,
    ...
    mod_row("AI生涯规划", "覆盖学生", career["total_users"], "人", "已获取", "网站后台", "AI生涯规划记录 > 互动记录", "网站汇总卡片：使用总人数。")
    mod_row("AI生涯规划", "累计使用次数", career["total_uses"], "次", "已获取", "网站后台", "AI生涯规划记录 > 互动记录", "网站汇总卡片：使用总次数。")
    mod_row("AI生涯规划", "本月使用次数", career["current_month_uses"], "次", "已获取", "网站后台", "AI生涯规划记录 > 互动记录", "网站汇总卡片：本月使用总次数。")
    mod_row("AI生涯规划", "后台列表记录数", career["table_total_rows"], "条", "已获取", "网站后台", "AI生涯规划记录 > 互动记录", "分页总数；与使用次数不同。")
    mod_row("AI生涯规划", "Excel功能使用量", total_row["ai_career_data_count"], "次", "已获取", "Excel", "总计行：AI生涯规划数据条数", "Excel导出功能使用量。")

    partner_history = manual["ai_partner_history"]
    partner_content = api["partner_content_stats"][0]
    mod_row("AI心语伙伴", "累计对话", partner_history["total_uses"], "次", "已获取", "网站后台", "AI心语伙伴 > 历史分析", "历史分析口径，适合用于“累计对话”。")
    mod_row("AI心语伙伴", "覆盖学生", partner_history["total_users"], "人", "已获取", "网站后台", "AI心语伙伴 > 历史分析", "历史分析口径，适合用于“覆盖学生”。")
    mod_row("AI心语伙伴", "内容统计用户数", partner_content["user_count"], "人", "已获取", "网站后台", "AI心语伙伴 > 内容统计", "心理内容统计口径，不与历史分析覆盖学生混写。")
    mod_row("AI心语伙伴", "已分析人数", partner_content["analysis_count"], "人", "已获取", "网站后台", "AI心语伙伴 > 内容统计", "心理分析已覆盖人数。")
    mod_row("AI心语伙伴", "AI画像数", manual["ai_partner_profile"]["table_total_rows"], "条", "已获取", "网站后台", "AI画像列表", "AI画像列表分页总数。")
    mod_row("AI心语伙伴", "Excel功能使用量", total_row["ai_partner_data_count"], "次", "已获取", "Excel", "总计行：AI心语伙伴数据条数", "Excel导出功能使用量。")

    bottle_delta_note = ""
    if bottle_latest_total and bottle_latest_total != bottle_stats["bottle_count"]:
        bottle_delta_note = f"全量审计列表记录为 {bottle_latest_total}，与主聚合快照不同，可能是实时新增或口径差异。"
    mod_row("漂流瓶", "发布数量", bottle_stats["bottle_count"], "条", "已获取", "网站后台", "漂流瓶管理 > 发布记录 API", "主聚合快照中的发瓶数量。", note=bottle_delta_note)
    mod_row("漂流瓶", "回复数量", bottle_stats["reply_count"], "条", "已获取", "网站后台", "漂流瓶管理 > 发布记录 API", "主聚合快照中的回复数量。")
    mod_row("漂流瓶", "累计收发", bottle_stats["bottle_count"] + bottle_stats["reply_count"], "条", "公式生成", "网站后台/计算", "发布数量 + 回复数量", "漂流瓶累计收发量。", "发布数量 + 回复数量")
    mod_row("漂流瓶", "回复率", bottle_stats["reply_count"] / bottle_stats["bottle_count"] if bottle_stats["bottle_count"] else None, "%", "公式生成", "网站后台/计算", "回复数量 / 发布数量", "以主聚合快照为准。", "回复数量 / 发布数量")
    mod_row("漂流瓶", "本月漂流瓶数量", bottle_stats["month_bottle_count"], "条", "已获取", "网站后台", "漂流瓶管理 > 发布记录 API", "主聚合快照中的本月发瓶数量。")
    ...
    mod_row("漂流瓶", "Excel功能使用量", total_row["bottle_send_count"] + total_row["bottle_reply_count"], "次", "已获取", "Excel/计算", "总计行：漂流瓶发送数量 + 漂流瓶回复数量", "Excel功能模块比较口径。")

    interview = api["ai_interview_info"]
    interview_reports = safe_get(api, "ai_interview_reports_meta", "numeric_total_like_fields", "data.pagination.total")
    mod_row("AI模拟面试", "服务学生", interview["total_users"], "人", "已获取", "网站后台", "AI模拟面试统计 API", "网站接口 total_users。")
    mod_row("AI模拟面试", "累计使用人次", interview["total_records"], "次", "已获取", "网站后台", "AI模拟面试统计 API", "网站接口 total_records。")
    mod_row("AI模拟面试", "本月使用人次", interview["current_month_records"], "次", "已获取", "网站后台", "AI模拟面试统计 API", "网站接口 current_month_records。")
    mod_row("AI模拟面试", "生成报告", interview_reports, "份", "已获取", "网站后台", "/api/ai/interview_stats/reports 分页总数", "报告列表总数。")
    mod_row("AI模拟面试", "深度使用率", interview_reports / interview["total_records"] if interview["total_records"] else None, "%", "公式生成", "网站后台/计算", "生成报告 / 累计使用人次", "衡量从使用到报告生成的转化。", "生成报告 / 累计使用人次")
    mod_row("AI模拟面试", "Excel功能使用量", total_row["ai_interview_data_count"] + total_row["ai_report_count"], "次", "已获取", "Excel/计算", "总计行：AI面试数据条数 + AI输出报告数量", "Excel功能模块比较口径。")
    ...
    aggregate_note = "后台内容统计聚合口径，表示截至统计日期/采集时间的累计汇总；不是单独某一月份数据。"
    for row in planner.get("tags", []):
        grade = row.get("grade")
        content_row("AI生涯规划", "年级使用占比", grade, "使用人数占比", row.get("use_rate"), "%", "已获取", "planner_content_stats.tags", aggregate_note)
        for key, val in row.items():
            if key not in {"grade", "grade_id", "school_year", "use_rate"}:
                content_row("AI生涯规划", "年级互动内容分类", grade, key, val, "次", "已获取", "planner_content_stats.tags", aggregate_note)
    ...
    for row in partner_content.get("analysis_grade", []):
        ...
        for key, val in row.items():
            if key != "grade_name":
                ...
                content_row("AI心语伙伴", "心理状态年级分布", grade, public_name, val, "%", "已获取", "partner_content_stats.analysis_grade", aggregate_note)
    for row in partner_content.get("advices_stat", {}).get("grade", []):
        grade = row.get("0")
        for key, val in row.items():
            if key != "0":
                content_row("AI心语伙伴", "心理困扰因子年级分布", grade, key, val, "%", "已获取", "partner_content_stats.advices_stat.grade", aggregate_note)
    for idx, (key, val) in enumerate(partner_content.get("keywords_stat", {}).items(), start=1):
    ...
    qa("去重服务覆盖率公式", "通过", f"{served_students} / {student_accounts} = {pct_text(service_coverage_rate)}")
    qa("登录人次覆盖强度公式", "通过", f"{total_logins} / {student_accounts} = {pct_text(login_intensity_rate)}")
    ...
    excel_monthly_usage = []
    for row in excel.get("raw_rows", []):
        if row.get("month") == "总计":
            continue
        excel_monthly_usage.append(
            {
                "month": row.get("month"),
                "total_user_count": row.get("total_user_count"),
                "total_login_count": row.get("total_login_count"),
                "ai_career_data_count": row.get("ai_career_data_count"),
                "ai_partner_data_count": row.get("ai_partner_data_count"),
                "message_bottle_total": (row.get("bottle_send_count") or 0) + (row.get("bottle_reply_count") or 0),
                "ai_interview_total": (row.get("ai_interview_data_count") or 0) + (row.get("ai_report_count") or 0),
            }
        )
    ...
    return {
        ...
        "excel_monthly_usage": excel_monthly_usage,
...
def write_registry_xlsx(registry):
    ...
    ws.cell(row=core_row_by_id["CORE-005"], column=3).value = f'=IFERROR(C{core_row_by_id["CORE-006"]}/C{core_row_by_id["CORE-003"]},0)'
    ws.cell(row=core_row_by_id["CORE-005"], column=3).number_format = "0.00%"
    ws.cell(row=core_row_by_id["CORE-012"], column=3).value = f'=IFERROR(C{core_row_by_id["CORE-004"]}/C{core_row_by_id["CORE-003"]},0)'
    ws.cell(row=core_row_by_id["CORE-012"], column=3).number_format = "0.00%"
...
def main():
    ...
    if validate_website_payload:
        website_issues = validate_website_payload(website, excel.get("school_name"))
        if website_issues:
            raise RuntimeError(
                "website_data.json failed validation; do not generate an empty report. "
                "Ask the user to complete website login/data collection first. Issues: "
                + "; ".join(website_issues)
            )
